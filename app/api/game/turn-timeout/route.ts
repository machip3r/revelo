...
import { pickTimeoutGuess } from "@/lib/botGuess";
...
export async function POST(request: Request) {
  ...
  const { data: room, error: roomErr } = await admin
    .from("rooms")
    .select("id, status, current_turn_player_id, turn_deadline_at")
    .eq("id", roomId)
    .single();
  ...
  if (Date.now() <= new Date(room.turn_deadline_at).getTime()) {
    return NextResponse.json({ ok: true, skipped: true });
  }
  ...
  const { data: claimed, error: claimErr } = await admin
    ...
    .eq("id", roomId)
    .eq("current_turn_player_id", attacker.id)
    .not("turn_deadline_at", "is", null)
    .lt("turn_deadline_at", cutoff)
    .select("id");
  ...
  const aliveOthers = allPlayers
    .filter((p) => p.is_alive && p.id !== attacker.id)
    .map((p) => p.id);
  ...
  const { targetId, guess } = pickTimeoutGuess({
    attackerId: attacker.id,
    aliveOtherIds: aliveOthers,
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : "Timeout resolution failed";
